...
bool bad_Link = false; // RF link status
...
void retry_ON()
{
  ...
  while ( !ok )
  {
    ...
    rton++;
    if ( rton == 16 )      // 15 * 200 = 3000msec = 3 sec = link not working
    {
      bad_Link = true; // Sets the bad link flag
      ok = true; // Forces loop to exit
    }
  }
  // If we get here then the command completed
  LED_1_OFF();
}
...
void retry_OFF()
{
  ...
  while ( !ok )
  {
    ...
    delay(200); // Wait 200ms before retrying command
    rtoff++;
    if ( rtoff == 16 )      // 15 * 200 = 3000msec = 3 sec = link not working
    {
      bad_Link = true; // Sets the bad link flag
      ok = true; // Forces loop to exit
    }
...
void runDelay()
{
  ...
  // Test to see if there was a hard link failure
  if ( ! bad_Link )
  {
    // Command worked so continue on
    LED_2_ON();
  ...
  bad_Link = false; // Reset bad link flag.  Wait for another button press.
}
...
void loop(void)
{
  ...
  while (1)
  {
    ...
    {     
      ...
      bad_Link = false; // Reset bad link flag
    }
